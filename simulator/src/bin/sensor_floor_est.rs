...
fn run_single_experiment(params: &ExperimentParams) -> (f64, f64, (usize, usize)) {
    ...
    // Calculate average error, or NaN if no detections
    let avg_err = if detection_count > 0 {
        total_err / detection_count as f64
    } else {
        f64::NAN
    };

    // Calculate detection rate
    let detection_rate = detection_count as f64 / params.experiment_count as f64;

    // Return avg error, detection rate, and indices
    (avg_err, detection_rate, params.indices)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let disks = Array1::range(2.0, 8.25, 0.25);

    // Star magnitudes to test - 15 to 20 in steps of 0.25
    let mags = Array1::range(10.0, 20.25, 0.25);

    println!("Setting up experiments...");

    // Store results for each sensor
    let sensor_results: Arc<Mutex<HashMap<String, Array2<f64>>>> =
    ...
    for sensor in &all_sensors {
        ...
        sensor_results
            .lock()
            .unwrap()
            .insert(sensor.name.clone(), sensor_results_array);
    ...
    let detection_rates: Arc<Mutex<HashMap<String, Array2<f64>>>> =
        Arc::new(Mutex::new(HashMap::new()));
    ...
    for sensor in &all_sensors {
        ...
        detection_rates
            .lock()
            .unwrap()
            .insert(sensor.name.clone(), detection_rate_array);
    }
    ...
    let results: Vec<_> = all_experiments
        .par_iter()
        .map(|params| {
            let sensor_name = params.sensor.name.clone();
            let (err, detection_rate, indices) = run_single_experiment(params);
            pb.inc(1);
            (sensor_name, err, detection_rate, indices)
        })
        .collect();
    ...
    for (sensor_name, err, detection_rate, (disk_idx, mag_idx)) in results {
        let mut sensor_results_map = sensor_results.lock().unwrap();
        if let Some(sensor_array) = sensor_results_map.get_mut(&sensor_name) {
            sensor_array[[disk_idx, mag_idx]] = err;
        }

        let mut detection_rates_map = detection_rates.lock().unwrap();
        if let Some(detection_array) = detection_rates_map.get_mut(&sensor_name) {
    ...
    // Get the maps from Arc<Mutex>
    let sensor_results_map = sensor_results.lock().unwrap();
    let detection_rates_map = detection_rates.lock().unwrap();
    ...
    for (sensor_name, results) in sensor_results_map.iter() {
        let detection_rate_array = detection_rates_map.get(sensor_name).unwrap();
        ...
        println!("\n  Mean Position Error Matrix (pixels, NaN = no detection):");

        // Write error matrix title to CSV
        writeln!(csv_file, "Mean Position Error Matrix (pixels)").unwrap();
